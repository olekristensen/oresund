...
float centerAndNormalize(ofMesh& mesh, ofVec3f cornerMin, ofVec3f cornerMax) {
    ...
    ofLogNotice("centerAndNormalize") << scale << endl;
...
float centerAndNormalize(ofMesh& mesh) {
	ofVec3f cornerMin, cornerMax;
	getBoundingBox(mesh, cornerMin, cornerMax);
    return centerAndNormalize(mesh, cornerMin, cornerMax);
}
